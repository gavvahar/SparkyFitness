...
function extractBodyFatValue(record: unknown): number | null {
  const r = record as Record<string, unknown>;
  const percentage = r.percentage as
    | Record<string, unknown>
    | number
    | undefined;
  const bodyFatPercentage = r.bodyFatPercentage as
    | Record<string, unknown>
    | undefined;
  ...
  if (
    ...
    percentage !== null &&
    'inPercent' in percentage
  ) {
    return percentage.inPercent as number;
  }
  if (
    typeof bodyFatPercentage === 'object' &&
    bodyFatPercentage !== null &&
    'inPercent' in bodyFatPercentage
  ) {
...
function extractVo2Value(record: unknown): number | null {
  ...
  if (typeof r.vo2MillilitersPerMinuteKilogram === 'number')
    return r.vo2MillilitersPerMinuteKilogram;
  return null;
}

function extractBMRValue(record: unknown): number | null {
  const r = record as Record<string, unknown>;
  const basalMetabolicRate = r.basalMetabolicRate as
    | Record<string, unknown>
    | number
    | undefined;
  ...
  if (basalMetabolicRate !== undefined) {
    if (typeof basalMetabolicRate === 'number') {
      ...
      typeof basalMetabolicRate === 'object' &&
      basalMetabolicRate !== null
    ) {
      if ('inKilocaloriesPerDay' in basalMetabolicRate)
        return basalMetabolicRate.inKilocaloriesPerDay as number;
      if ('inCalories' in basalMetabolicRate)
        return basalMetabolicRate.inCalories as number;
      if ('inKilocalories' in basalMetabolicRate)
        return basalMetabolicRate.inKilocalories as number;
      if ('value' in basalMetabolicRate)
        return basalMetabolicRate.value as number;
    }
  } else {
...
const AGGREGATED_FORMATTERS: Record<
  string,
  (start: Date, end: Date) => Promise<string>
> = {
  Steps: makeAggregatedFormatter(getAggregatedStepsByDate, t =>
    t.toLocaleString(),
  ),
  ActiveCaloriesBurned: makeAggregatedFormatter(
    getAggregatedActiveCaloriesByDate,
    t => t.toLocaleString(),
  ),
  TotalCaloriesBurned: makeAggregatedFormatter(
    getAggregatedTotalCaloriesByDate,
    t => t.toLocaleString(),
  ),
  Distance: makeAggregatedFormatter(
    getAggregatedDistanceByDate,
    t => `${(t / 1000).toFixed(2)} km`,
  ),
  FloorsClimbed: makeAggregatedFormatter(getAggregatedFloorsClimbedByDate, t =>
...
const RAW_FORMATTERS: Record<string, (records: unknown[]) => string> = {
  ...
  Weight: records => {
    const latestWeight = (
      records as { time: string; weight?: { inKilograms: number } }[]
    ).sort(
  ...
  Hydration: records => {
    const totalHydration = (
      records as { volume?: { inLiters: number } }[]
  ...
  Height: records => {
    const latestHeight = (
      records as { time: string; height?: { inMeters: number } }[]
    ).sort(
      (a, b) => new Date(b.time).getTime() - new Date(a.time).getTime(),
    )[0];
  ...
  BloodGlucose: records => {
    ...
    const glucoseValue =
      latestGlucose.level?.inMillimolesPerLiter ||
      latestGlucose.bloodGlucose?.inMillimolesPerLiter ||
      (latestGlucose.level?.inMilligramsPerDeciliter
        ? latestGlucose.level.inMilligramsPerDeciliter / 18.018
        : null) ||
      (latestGlucose.bloodGlucose?.inMilligramsPerDeciliter
        ? latestGlucose.bloodGlucose.inMilligramsPerDeciliter / 18.018
        : null);
    return glucoseValue ? `${glucoseValue.toFixed(1)} mmol/L` : NO_DATA_DISPLAY;
  },

  OxygenSaturation: records =>
    formatLatestPercentValue(records, extractO2Value, v => v > 0 && v <= 100),
  BloodOxygenSaturation: records =>
    formatLatestPercentValue(records, extractO2Value, v => v > 0 && v <= 100),

  RestingHeartRate: records => {
    const avgRestingHR =
      (records as { beatsPerMinute?: number }[]).reduce(
        (sum, record) => sum + (record.beatsPerMinute || 0),
        0,
      ) / records.length;
    return avgRestingHR > 0
      ? `${Math.round(avgRestingHR)} bpm`
      : NO_DATA_DISPLAY;
  },
  ...
  Vo2Max: records => {
    ...
    return valid.length > 0
      ? `${valid[0].value!.toFixed(1)} ml/min/kg`
      : NO_DATA_DISPLAY;
  },
  ...
  BasalMetabolicRate: records => {
    ...
    const aggregatedBMR = Object.values(dailyBMRs).map(
  ...
  ExerciseSession: records => {
    ...
    return totalExerciseMinutes === 0
      ? NO_DATA_DISPLAY
      : `${Math.round(totalExerciseMinutes)} min`;
  },

  ElevationGained: records => {
    const totalElevation = (
      records as { elevation?: { inMeters: number } }[]
    ).reduce((sum, record) => sum + (record.elevation?.inMeters || 0), 0);
  ...
  Power: records => {
    const avgPower =
      (records as { power?: { inWatts: number } }[]).reduce(
        (sum, record) => sum + (record.power?.inWatts || 0),
        0,
      ) / records.length;
  ...
  Speed: records => {
    const avgSpeed =
      (records as { speed?: { inMetersPerSecond: number } }[]).reduce(
        (sum, record) => sum + (record.speed?.inMetersPerSecond || 0),
        0,
      ) / records.length;
  ...
  RespiratoryRate: records => {
    const avgRespRate =
      (records as { rate?: number }[]).reduce(
        (sum, record) => sum + (record.rate || 0),
        0,
      ) / records.length;
    return avgRespRate === 0
      ? NO_DATA_DISPLAY
      : `${Math.round(avgRespRate)} br/min`;
  },

  Nutrition: records => {
    const totalNutrition = (
      records as { energy?: { inCalories: number } }[]
    ).reduce((sum, record) => sum + (record.energy?.inCalories || 0), 0);
    return totalNutrition === 0
      ? NO_DATA_DISPLAY
      : `${Math.round(totalNutrition / 1000)} kcal`;
  },
...
export async function fetchHealthDisplayData(
  ...
  for (const metric of HEALTH_METRICS) {
    try {
      ...
      addLog(
        `[healthDataDisplay] Error fetching ${metric.label}: ${errorMessage}`,
        'ERROR',
      );
