...
export function useFoodVariants(
  foodId: string,
...
export function useCreateFoodVariant() {
  ...
  const mutation = useMutation({
    mutationFn: (payload: CreateFoodVariantPayload) =>
      createFoodVariant(payload),
    onSuccess: (_variant, payload) => {
      void queryClient.invalidateQueries({
        queryKey: foodVariantsQueryKey(payload.food_id),
        refetchType: 'all',
      });
      void queryClient.invalidateQueries({
      ...
      void queryClient.invalidateQueries({
        queryKey: foodSearchQueryKey(''),
        refetchType: 'inactive',
      });
      void queryClient.invalidateQueries({
        queryKey: foodsLibraryQueryKey(''),
        refetchType: 'inactive',
      });
    },
  });
...
export function useDeleteFoodVariant() {
  ...
  const mutation = useMutation({
    mutationFn: ({ variantId }: { variantId: string; foodId: string }) =>
      deleteFoodVariant(variantId),
  ...
  return {
    deleteVariant: mutation.mutateAsync,
    isPending: mutation.isPending,
  };
}
