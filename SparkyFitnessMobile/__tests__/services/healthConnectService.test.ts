...
jest.mock('../../src/HealthMetrics', () => ({
  HEALTH_METRICS: [
    ...
    {
      recordType: 'Distance',
      stateKey: 'isDistanceSyncEnabled',
      unit: 'meters',
      type: 'distance',
    },
    {
      recordType: 'FloorsClimbed',
      stateKey: 'isFloorsClimbedSyncEnabled',
      unit: 'count',
      type: 'floors_climbed',
    },
  ],
...
const androidService =
  require('../../src/services/healthConnectService.ts') as {
...
describe('healthConnectService.ts (Android)', () => {
  ...
  describe('syncHealthData', () => {
    ...
    test('Steps are aggregated via native aggregateGroupByPeriod (cross-origin dedup)', async () => {
      ...
      const payload = mockApiSyncHealthData.mock.calls[0][0];
      const stepRecords = payload.filter(
        (r: { type: string }) => r.type === 'step',
      );
    ...
    test('ActiveCalories are aggregated via native aggregateGroupByPeriod', async () => {
      ...
      const healthMetricStates: HealthMetricStates = {
        isCaloriesSyncEnabled: true,
      };

      await androidService.syncHealthData('24h', healthMetricStates);

      const payload = mockApiSyncHealthData.mock.calls[0][0];
      const calorieRecords = payload.filter(
        (r: { type: string }) => r.type === 'active_calories',
      );
    ...
    test('HeartRate records are aggregated with min/max/avg by date', async () => {
      ...
      const payload = mockApiSyncHealthData.mock.calls[0][0];
      const hrMin = payload.find(
        (r: { type: string }) => r.type === 'heart_rate_min',
      );
      const hrMax = payload.find(
        (r: { type: string }) => r.type === 'heart_rate_max',
      );
      const hrAvg = payload.find(
    ...
    test('continues sync when one metric returns no records', async () => {
      ...
      const result = await androidService.syncHealthData(
        '24h',
        healthMetricStates,
      );
      ...
      const payload = mockApiSyncHealthData.mock.calls[0][0];
      expect(
        payload.some((r: { type: string }) => r.type.startsWith('heart_rate_')),
      ).toBe(true);
      expect(payload.some((r: { type: string }) => r.type === 'step')).toBe(
        false,
      );
    });
    ...
    test('Distance is aggregated via native aggregateGroupByPeriod', async () => {
      ...
      const healthMetricStates: HealthMetricStates = {
        isDistanceSyncEnabled: true,
      };

      await androidService.syncHealthData('24h', healthMetricStates);

      const payload = mockApiSyncHealthData.mock.calls[0][0];
      const distanceRecords = payload.filter(
        (r: { type: string }) => r.type === 'distance',
      );
    ...
    test('FloorsClimbed is aggregated via native aggregateGroupByPeriod', async () => {
      ...
      const healthMetricStates: HealthMetricStates = {
        isFloorsClimbedSyncEnabled: true,
      };

      await androidService.syncHealthData('24h', healthMetricStates);

      const payload = mockApiSyncHealthData.mock.calls[0][0];
      const floorsRecords = payload.filter(
        (r: { type: string }) => r.type === 'floors_climbed',
      );
  ...
  describe('per-day UTC offset metadata', () => {
    ...
    test('sync sends offset metadata through the full pipeline for TotalCalories', async () => {
      ...
      const payload = mockApiSyncHealthData.mock.calls[0][0];
      const calRecords = payload.filter(
        (r: { type: string }) => r.type === 'total_calories',
      );
